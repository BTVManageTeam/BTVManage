...
public class JsonUtil {
	...
	public static String toJson(final Object o) {
		...
		try {
			throw new MyDefineException(StringUtil.format("不支持的类型： {}", o.getClass().getName()));
	...
	public static String object2Json(final Object bean) {
		...
		for (final Method method : methods) {
			try {
				...
				logger.error("在将bean封装成JSON格式时异常：" + e.getMessage());
			}
		}
		if (sb.length() == 1) {
	...
	public static JSONArray Json2JSONArray(String s) {
		...
		try {
			...
			logger.error("String转JSONArray异常：" + e.getMessage());
		}
		return array;
	}
	 ...
	 * @return
	 */
	public static JSONObject Json2Obj(String s) {
		JSONObject obj = null;
		try {
